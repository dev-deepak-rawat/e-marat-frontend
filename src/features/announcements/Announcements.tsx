...
export default function Announcements() {
    ...
    useEffect(() => {
        const fetchAnnouncements = async () => {
            setLoading(true)
            const announcementsList = await apiRequest({ apiUrl: 'announcements' });
    ...
    return (
        <List
            ...
            renderItem={(item) => {
                ...
                return <List.Item className="mx-auto w-4/5">
                    <Comment
                        datetime={
                            <Tooltip title={format(createdAtDate, 'yyyy-MM-dd HH:mm:ss')}>
                        ...
                        content={
                            <>
                                {picture && <img src={transformCloudinaryImage(picture, 'WIDTH_500')} alt="announcement" />}
